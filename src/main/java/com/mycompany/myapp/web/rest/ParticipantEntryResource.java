...
public class ParticipantEntryResource {
    ...
    private final ParticipantEntrySearchRepository participantEntrySearchRepository;

    public ParticipantEntryResource(ParticipantEntryRepository participantEntryRepository, ParticipantEntrySearchRepository participantEntrySearchRepository) {
        this.participantEntryRepository = participantEntryRepository;
        this.participantEntrySearchRepository = participantEntrySearchRepository;
    }
    ...
    public ResponseEntity<ParticipantEntry> createParticipantEntry(@RequestBody ParticipantEntry participantEntry) throws URISyntaxException {
        ...
        participantEntrySearchRepository.save(result);
    ...
    public ResponseEntity<Void> deleteParticipantEntry(@PathVariable Long id) {
        ...
        participantEntrySearchRepository.deleteById(id);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, true, ENTITY_NAME, id.toString())).build();
    }

    /**
     * {@code SEARCH  /_search/participant-entries?query=:query} : search for the participantEntry corresponding
     * to the query.
     *
     * @param query the query of the participantEntry search.
     * @return the result of the search.
     */
    @GetMapping("/_search/participant-entries")
    public List<ParticipantEntry> searchParticipantEntries(@RequestParam String query) {
        log.debug("REST request to search ParticipantEntries for query {}", query);
        return StreamSupport
            .stream(participantEntrySearchRepository.search(queryStringQuery(query)).spliterator(), false)
